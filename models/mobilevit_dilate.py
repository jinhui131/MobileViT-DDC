...
class MobileViTDilateBlock(nn.Module):
    def __init__(
        self,
        in_channels: int,
        transformer_dim: int,
        ...
        **kwargs
        ...
        # Replacing global transformer representation with custom DilateBlock
...
class MobileViTDilate(nn.Module):
    """
    ...
    def _make_mit_layer(input_channel: int, cfg: Dict) -> [nn.Sequential, int]:
        ...
        block.append(MobileViTDilateBlock(
            in_channels=input_channel,
            ...
            dropout=cfg.get("dropout", 0.1),#0.1
            ffn_dropout=cfg.get("ffn_dropout", 0.0),#0.0
            attn_dropout=cfg.get("attn_dropout", 0.1),#0.1
            head_dim=head_dim,
            conv_ksize=3
        ))
...
def mobile_vit_dilate_xx_small(num_classes: int = 1000):
...
def mobile_vit_dilate_x_small(num_classes: int = 1000):
    # pretrain weight link
    # https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_xs.pt
    config = get_config("x_small")
    model  = MobileViTDilate(config, num_classes=num_classes)
    return model


def mobile_vit_dilate_small(num_classes: int = 1000):
    # pretrain weight link
